...
| **Mounts** | The home directory, the context root, the internal state directory and PartCAD's own installation, mounted at the paths they have on the host (drive-letter-mapped on Windows) and all writable. Nested paths are dropped, so on an ordinary machine the home directory covers the other three and there is one mount. Nothing may occupy those paths in the image. The container is named after the image alone, so it is reused across contexts and across runs; only a new version or tag makes a new one. |
