...
| **Mounts** | The context root, the internal state directory and PartCAD's own installation are mounted at the paths they have on the host (drive-letter-mapped on Windows). The installation is read-only, unless it sits inside one of the other two, which are writable and mount it as part of themselves. The same precedence runs the other way for the directories an ad-hoc command names: write access asked for anywhere in a subtree makes the mount covering it writable. Nothing may occupy those paths in the image. The container is named after the image and this mount set together, so a container answering to a name has the mounts that name asks for and two contexts never contend for one. |
