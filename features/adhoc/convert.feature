...
Feature: `pc adhoc convert` command
  ...
  Scenario: Convert STL to STEP
    When I run "pc adhoc convert test.stl test.step"
    Then the command should exit with a status code of "0"
    And STDOUT should contain "Ad-hoc converting 'test.stl': STL to STEP"
    And STDOUT should contain "Ad-hoc conversion successful: test.step"
  ...
  Scenario: Ambiguous input file extension
    ...
    Then the command should exit with a status code of "1"
    And STDOUT should contain "Unknown input extension 'unknown'. Please specify --input."
