...
Feature: `pc config` command
  ...
  Scenario: Override CLI options with environment variables
    When I run "PC_THREADS_MAX=1234 PC_OLLAMA_NUM_THREAD=5678 PC_MAX_SCRIPT_CORRECTION=4321 PC_MAX_MODEL_GENERATION=8765 PC_MAX_GEOMETRIC_MODELING=3456 PC_OPENAI_API_KEY=abcdef12345 PC_GOOGLE_API_KEY=fedcba54321 PC_INTERNAL_STATE_DIR=/tmp/sandbox/home/.partcad PC_PYTHON_SANDBOX=conda PC_FORCE_UPDATE=false PC_CACHE_FILES=false PC_CACHE_FILES_MAX_ENTRY_SIZE=101 PC_CACHE_FILES_MIN_ENTRY_SIZE=11 PC_CACHE_MEMORY_MAX_ENTRY_SIZE=101 PC_CACHE_MEMORY_DOUBLE_CACHE_MAX_ENTRY_SIZE=101 PC_CACHE_DEPENDENCIES_IGNORE=false pc --threads-max=1111 --ollama-num-thread=2222 --max-script-correction=3333 --max-model-generation=4444 --max-geometric-modeling=5555 --openai-api-key=notused --google-api-key=notused --internal-state-dir=/not/used --python-runtime=pypy --force-update --cache --cache-max-entry-size=102 --cache-min-entry-size=12 --cache-memory-max-entry-size=102 --cache-memory-double-cache-max-entry-size=102 --cache-dependencies-ignore config"
    Then the command should exit with a status code of "0"
    And STDOUT should contain "threads_max: 1234"
    And STDOUT should contain "ollama_num_thread: 5678"
    And STDOUT should contain "max_script_correction: 4321"
  ...
  Scenario: Environment variables should have the highest precedence
    Given a user configuration file named "config.yaml" with content:
      ...
      cacheDependenciesIgnore: false
      """
    When I run "PC_THREADS_MAX=1234 PC_OLLAMA_NUM_THREAD=5678 PC_MAX_SCRIPT_CORRECTION=4321 PC_MAX_MODEL_GENERATION=8765 PC_MAX_GEOMETRIC_MODELING=3456 PC_OPENAI_API_KEY=abcdef12345 PC_GOOGLE_API_KEY=fedcba54321 PC_INTERNAL_STATE_DIR=/tmp/sandbox/home/.partcad PC_PYTHON_SANDBOX=pypy PC_FORCE_UPDATE=false PC_CACHE_FILES=true PC_CACHE_FILES_MAX_ENTRY_SIZE=101 PC_CACHE_FILES_MIN_ENTRY_SIZE=11 PC_CACHE_MEMORY_MAX_ENTRY_SIZE=101 PC_CACHE_MEMORY_DOUBLE_CACHE_MAX_ENTRY_SIZE=101 PC_CACHE_DEPENDENCIES_IGNORE=true pc --threads-max=1111 --ollama-num-thread=2222 --max-script-correction=3333 --max-model-generation=4444 --max-geometric-modeling=5555 --openai-api-key=notused --google-api-key=notused --internal-state-dir=/not/used --python-runtime=none --force-update --cache-max-entry-size=102 --cache-min-entry-size=12 --cache-memory-max-entry-size=102 --cache-memory-double-cache-max-entry-size=102 config"
    Then the command should exit with a status code of "0"
    And STDOUT should contain "threads_max: 1234"
    And STDOUT should contain "ollama_num_thread: 5678"
    And STDOUT should contain "max_script_correction: 4321"
