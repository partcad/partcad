...
def run(context: Context, command: str):
    if command.startswith("pc "):
        command = (
            f"coverage run --data-file=$PARTCAD_ROOT/.coverage --parallel -m partcad_cli.click.command {command[3:]}"
        )
    elif command.startswith("partcad "):
        command = (
            f"coverage run --data-file=$PARTCAD_ROOT/.coverage --parallel -m partcad_cli.click.command {command[8:]}"
        )
