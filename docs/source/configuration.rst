...
numbers are in, which applications it declares, and -- under ``Layers`` --
**every** layer the file has, with how many elements of which types are on each
and whether this sketch reads it. That last one is not the same question the
annotations answer, and it is why the drawing is described as well as read: a
layer filter that matched nothing and a layer that is not in the file both
produce a sketch with nothing in it.
...
with the keys lower-cased the same way. Neither reading needs a CAD kernel:
both formats are text, and PartCAD reads them off the bytes.
