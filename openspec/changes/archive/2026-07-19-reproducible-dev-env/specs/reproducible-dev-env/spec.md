...
### Requirement: The headless entry point documents what the editor integration provides implicitly
The VS Code Dev Containers extension supplies several things automatically that the command line entry point
does not. Documentation of the headless flow SHALL cover each of them, so a contributor or agent using it is not
blocked by a gap the editor workflow hides: the git identity, commit signing, and the fact that state written to
the container's home directory does not survive recreating the container.

#### Scenario: Committing without a git identity
- **WHEN** a commit inside the environment fails with `Author identity unknown`
- **THEN** the documentation explains that the headless entry point does not copy the host git configuration,
  and directs the contributor to set the identity where it survives container recreation (the repository-local
  `.git/config` in the bind-mounted workspace)

#### Scenario: Committing with signing enabled
- **WHEN** a contributor has `commit.gpgsign` enabled and a commit inside the environment fails to sign
- **THEN** the documentation explains that the headless entry point does not forward the GPG agent, and
  describes forwarding the agent's socket so that the private key remains on the host

#### Scenario: A contributor mounts the host git configuration
- **WHEN** documentation addresses supplying the git identity to the container
- **THEN** it warns that mounting the host `~/.gitconfig` wholesale can break container startup, because
  `url.*.insteadOf` rules rewriting HTTPS to SSH cause the `git-lfs` feature's post-create step to fail when no
  SSH key is present in the container
