...
# in the order a reader wants it: what it is made of, how it is made or who it is
# bought from, and where its geometry is. Each is (heading, key, alignment).
#
...
def _bom_cell(entry, key):
    ...
    if key == "process":
        # One cell rather than a column per parameter: which parameters a process
        # has is the process's business, and a table with a column for "infill"
        # would have an empty one on every machined part.
        return "; ".join("%s %s" % (name, value[name]) for name in value)
    if key == "tolerance":
