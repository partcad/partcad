...
def sdf_triangles_to_topods(triangles: np.ndarray):
    """Build a triangulation-backed TopoDS_Face from a flat list of vertices.
    ...
    if arr.size == 0:
        return None
    if arr.ndim != 2 or arr.shape[1] != 3 or arr.shape[0] % 3 != 0:
        raise ValueError(
            "SDF tessellation produced %s vertices; expected an Nx3 array with N a multiple of 3" % (arr.shape,)
        )
    ...
    for i in range(n_tris):
        tris.SetValue(i + 1, Poly_Triangle(i * 3 + 1, i * 3 + 2, i * 3 + 3))
...
def _sdf_to_shape(obj, step, samples, batch_size, sparse):
    """Tessellate a single SDF3 object into a triangulation-backed TopoDS_Face."""
    points = obj.generate(
        step=step,
        samples=samples,
    ...
    return sdf_triangles_to_topods(tri_points)
    ...
    try:
        ...
        for result in build_result.results:
            ...
            if isinstance(shape, sdf.d3.SDF3):
                shape_3d = _sdf_to_shape(shape, step, samples, batch_size, sparse)
                ...
                def unpack(objs):
                    for obj in objs:
                        if isinstance(obj, sdf.d3.SDF3):
                            shape_3d = _sdf_to_shape(obj, step, samples, batch_size, sparse)
                            if shape_3d is not None:
                                yield shape_3d
                        elif is_vector(obj):
