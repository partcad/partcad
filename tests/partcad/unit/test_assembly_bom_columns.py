...
MACHINED = {
    "type": "step",
    "path": "plate.step",
    "manufacturing": {"method": "subtractive", "desc": "anodized"},
...
def test_line_reads_the_manufacturing_data():
    ...
    assert line["process"] == "anodized"
    assert line["source_file"] == "plate.step"
    assert line["vendor"] is None and line["sku"] is None


def test_how_the_part_is_made_is_printed_beside_the_method():
    """'manufacturing: desc:' is what a shop is told and the geometry cannot carry."""
    printed = {
        "type": "step",
        "manufacturing": {
            "method": "additive",
            "desc": "0.4 mm nozzle, 0.2 mm layer height, 45% infill",
        },
    }
    rows, _columns = _cells(_Part("finger", printed))
    assert rows["finger"]["Process"] == "0.4 mm nozzle, 0.2 mm layer height, 45% infill"
...
def test_columns_appear_where_there_is_something_to_say():
    ...
    assert table.rows[0][:6] == ["plate", 1, "//pub:aluminium-5052", "subtractive", "anodized", "`plate.step`"]
