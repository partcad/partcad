...
# --------------------------------------------------------------------------- #
# Where the throwaway package lives                                            #
# --------------------------------------------------------------------------- #


def test_the_throwaway_package_is_built_under_the_state_directory(stub_context, tmp_path, monkeypatch):
    """Because it is the context root, and a container sandbox mounts that.

    In the system temporary directory it was somewhere the home directory mount
    does not reach, so every ad-hoc command asked for one mount no previous one
    had -- and a mount set that differs is a container that gets replaced. So
    'pc convert' threw away the warm container and started another every single
    time. Under '~/.partcad' it is inside a mount that is already there.

    Both runs are checked, because one run landing in the right place proves
    nothing about the mount set holding still; two landing under one directory
    that is already mounted is the property.
    """
    state = tmp_path / "state"
    monkeypatch.setattr(adhoc_base.user_config, "internal_state_dir", str(state))

    roots = []
    real_context = adhoc_base.Context

    def capture(*args, **kwargs):
        roots.append(pathlib.Path(kwargs.get("root_path", args[0] if args else "")))
        return _FakeContext(_FakeObject(errors=[], shape=None))

    monkeypatch.setattr(adhoc_base, "Context", capture)
    assert real_context is not capture  # the fixture's stub is replaced, not stacked

    for _ in range(2):
        with pytest.raises(RuntimeError):
            adhoc_convert.convert_cad_file(str(tmp_path / "in.step"), "step", str(tmp_path / "out.stl"), "stl")

    assert len(roots) == 2
    for root in roots:
        assert state in root.parents, "%s is not under %s" % (root, state)
    # Gone again afterwards: where it lives changed, not how long.
