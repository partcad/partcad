...
def test_locally_built_extensions_are_required():
    plan = build_plan(["a.b"], {}, local={"OpenVMP.partcad": "/tmp/partcad.vsix"})
    (entry,) = [item for item in plan["install"] if item["id"] == "OpenVMP.partcad"]
...
def test_the_shipped_policy_matches_the_shipped_recommendations():
    ...
    plan = resolve_extensions.load_plan(
        ...
        {"OpenVMP.partcad": "partcad.vsix", "PartCAD.partcad-ide-bootstrap": "bootstrap.vsix"},
    )
