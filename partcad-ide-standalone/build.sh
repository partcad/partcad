...
#   partcad-ide-standalone/build.sh --no-archive       # leave it unpacked
#   partcad-ide-standalone/build.sh --record           # pin what it downloaded
#
...
Options:
  ...
  --vsix <path>            The PartCAD extension to install. Defaults to
                           `partcad-ide-vscode/partcad.vsix`, built with `npm`
...
if [ "${INSTALL_EXTENSIONS}" = "1" ]; then
  ...
  if [ -z "${EXTENSION_VSIX}" ]; then
    ...
    if [ ! -f "${EXTENSION_VSIX}" ]; then
      ...
      (cd "${REPO_ROOT}/partcad-ide-vscode" && npm ci && npm run vsce-package)
    fi
  fi
...
if [ "${BRAND_ICONS}" = "1" ]; then
  if "${PYTHON}" "${SCRIPT_DIR}/tools/make_icons.py" \
    --svg "${REPO_ROOT}/partcad-ide-vscode/resources/logo.svg" \
