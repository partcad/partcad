...
#
"""Redirecting the public index to its unreleased branch ('develIndex').
...
its default branch holds, which is the released state. 'develIndex' redirects it
...
class FakeUserConfig:
    ...
    def __init__(self, devel_index=False, url_overrides=None):
...
@pytest.mark.parametrize("url", INDEX_URLS)
...
def test_the_index_is_redirected_to_the_devel_branch():
    imp = Import(
        ...
        FakeUserConfig(devel_index=True),
    )
    assert imp.import_revision == consts.DEVEL_INDEX_REVISION


def test_nothing_is_redirected_when_the_option_is_off():
    imp = Import(
        {"url": "https://github.com/partcad/partcad-index.git"},
        FakeUserConfig(devel_index=False),
    )
