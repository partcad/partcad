...
```bash
poetry run pytest partcad-cad-freecad -x -p no:error-for-skips -p no:warnings --dist no   # matches CI
```
...
```bash
poetry run black --check partcad-cad-freecad
poetry run flake8 partcad-cad-freecad
```
