...
`partcad` imports this package lazily, from `Shape.show()` and `Interface.show()` only. It is not published
separately: the source is symlinked into `partcad/src`, so it ships inside the `partcad` wheel and
`pip install partcad` is all that is needed. Nothing else needs it, and nothing here imports `partcad`, OCP, or
any other CAD library — by the time geometry reaches this package it has already been tessellated into glTF
inside a PartCAD sandbox.
