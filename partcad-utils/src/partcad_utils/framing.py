...
This is the same framing the Language Server Protocol uses, so the VS Code side
can read and write it with ``vscode-jsonrpc`` unchanged. The functions operate
on any binary file object, which keeps them trivially unit-testable with
``io.BytesIO``.
"""
